...
def processTweets(tweets):
    ...
    for tweet in tweets:
        if not tweet.id:
            continue
        temp_dict = {'tweet id': str(tweet.id), 'user id': str(tweet.user.id), 'text': tweet.text, 'lang': tweet.lang, 'user location': tweet.user.location,
        'user geo_enabled': tweet.user.geo_enabled, 'geo': tweet.coordinates, 'created at': str(tweet.created_at), 'hashtags': []}
